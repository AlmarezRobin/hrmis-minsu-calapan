...
class Pds_model extends Model
{
	...
	#region membership
	public function insert_membership($name){
		$membership = ['assoc_name' => $name, 'user_id' => $this->session->userdata('user_id')];
		return $this->db->table('pds_organization_membership')->insert($membership);
	}
	public function update_membership($name, $id){
		$membership = ['assoc_name' => $name, 'user_id' => $this->session->userdata('user_id')];
	...
	public function get_id(){
		return $this->db->table('pds_gov_issued_id_tbl')->where('user_id', $this->session->userdata('user_id'))->get_all();
	}

	public function insert_id($desc, $idno, $date, $place){
		$id = [
			'id_desc' => $desc,
			'idno' => $idno,
			'date_issued' => $date,
		...
		return $this->db->table('pds_gov_issued_id_tbl')->insert($id);

	}

	public function update_id($desc, $idno, $date, $place, $gid){
		$id = [
			'id_desc' => $desc,
			'idno' => $idno,
			'date_issued' => $date,
		...
		return $this->db->table('pds_gov_issued_id_tbl')->where('gov_issued_id', $gid)->update($id);
	}

	public function delete_id($id)
	{
		return $this->db->table('pds_gov_issued_id_tbl')->where('gov_issued_id', $id)->delete();
	}
